...
### Removed
- **Guild QA discipline — removed pending review (BREAKING, v6.1.0).** Deleted `agents/qa-strategist.md`, `agents/qa-tester.md`, `skills/qa`, `skills/qa-mindset`, `skills/qa-artifacts`, and the `maintenance` execution template. From `schema.sql`: the `coverage`, `inspection` and `inspection_coverage` tables, the `v_coverage_due` view, their four triggers, and the `qa-planning` / `qa-execution` / `e2e` capability words. The schema is now **21 tables, 25 views and 39 triggers**, and the roster is **12 members**.

  **`bug` and `v_open_bugs` are kept deliberately** — they are load-bearing outside QA. `gate-repairs` collects bugs alongside review findings and failed tasks, and `guild:verify-and-fix` files them; removing them would have cut the review gate. `e2e` went with `qa-tester` because that was its only owner, and a vocabulary word nobody covers matches nobody. **`agent.serial` stays**: no member carries `serial = 1` now, so G9's `serial-agent-double-booked` is quiet until one does, which is the right state for a rule that outlives whichever member happened to need it — the assertion is expressed against the flag, never against a member's name.

  The `maintenance` fixture and `expectations.md` §11 went with it; §12 renumbered to §11.
...
### Changed
- **Guild Plugin v6.1.0 — repairs are planned the way the build is planned (BREAKING).** `repair` used to be plain tickets the orchestrator wrote at `gate-repairs`: no requirements, no plan, no file sets, no disjointness assertion. That is thin for anything larger than a one-line fix, and it made repairs the only code the guild wrote without a plan anyone approved. The `standard` template now runs `gate-repairs → repair-spec → repair-plan → gate-repair-plan → repair`. `repair-spec` (`requirements`) writes a `plan` row bound to its own ticket saying what the approved repairs must achieve — **with no interview**, because the issues were discussed item by item at `gate-repairs` and the user's words are already in `gate.decision`. `repair-plan` (`architecture`) writes the plan and cuts the repair tickets with their `files` and `parallel_group`. `gate-repair-plan` approves **how** the chosen problems get fixed, which is a different question from `gate-repairs`' **which** problems matter. Rows 7–10 are one optional group: approve nothing and all four drop together.

  **The arithmetic changed: N + 12 nodes, 2N + 13 edges, and 3 gates** (was N + 9, 2N + 10, 2). `expectations.md` §4.c asserts it, and G4/G8's hardcoded node-key and gate vocabularies were widened for the three new keys — `dropped-gate` still requires only `gate-plan` and `gate-repairs`, since `gate-repair-plan` is droppable with its group.

  **Repair tickets stay under the same requirement.** `v_open_findings` derives `requirement_id` through `task_id`, so filing repairs under a new requirement would leave a finding and its fix answering to different requirements and G6's closure rules reasoning about the wrong one.

  **The third gate is not free, and the template says so.** §4 of `standard.md` used to argue against ever adding one: gates are where an unattended shift stops, so a requirement that reaches `gate-repairs` overnight now stops a second time before any repair is written — at 3am, not at the moment of the decision. That argument still holds and is rewritten as the stated price rather than deleted.

- **Guild — plan slices folded into the ticket as `task.files` (BREAKING).** The `plan_slice` table, its index, and `task.plan_slice_id` / `task.plan_slice` are gone; `task.files` (`TEXT NOT NULL DEFAULT '[]' CHECK (json_valid(files))`) carries the file set each ticket owns. A slice existed to carry exactly one thing the ticket could not — the disjointness assertion that lets two developers edit one working tree concurrently — and everything else about it was indirection. No view ever referenced `plan_slice`, which is why the structural surface was four lines; the cost was 282 prose references across 30 files. `implement` now fans out **per-task** and keys on the ticket (`REQ-007/implement.TASK-011`), which makes instantiation a direct join rather than a correlated subquery and makes `graph_node.task_id` agree with the node id by construction. The test-planner's `PLAN-NNN/test-plan` slice moved to `plan.task_id`, the column that already meant "a plan written FOR one ticket". §4.d's disjointness assertion is **stronger** than the slice version — it compares tickets directly.

- **Guild Plugin v6.0.0 — the schema is the tool (BREAKING).** The plugin is no longer a program. It is **a schema and a set of skills**: `plugins/guild/schema.sql` plus the knowledge that teaches a guild member to use it. The guild master's reasoning — *"when the turso CLI is already installed, we have a tool that can execute SQL; we don't need to build another tool that does the same thing"* — is the whole of the change. `tursodb` **is** the tool; members write their own SQL; `skills/warehouse/` is the guide at the door.
