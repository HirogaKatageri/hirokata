...
**Sequential dispatch for QA tester tasks:**
...
**Unlike developer tasks, `qa-tester` tasks must NOT be parallelized.** Spawn
qa-testers strictly **one at a time** — dispatch a single qa-tester, wait for it
to complete, then dispatch the next. Each tester launches the running product
(dev/preview server + Playwright), and concurrent testers would collide on the
same dev-server / Playwright port. Even when 3 or more `qa-tester` tasks are
pending for the same QA pass, run them sequentially, never in a shared batch.
Each tester still works a separate mission and writes to its own files. QA tasks
do NOT trigger auto-test (the qa-tester owns its own e2e specs) — see step 4.5.
