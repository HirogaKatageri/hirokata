...
Per-adapter setup (`adapter-node`'s env vars, `adapter-static`'s SPA fallback, and the
Cloudflare / Vercel / Netlify platform bindings) is in `references/adapters.md` — load it once
the table above has picked one.
