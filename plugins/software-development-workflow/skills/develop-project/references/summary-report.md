...
2. The skill reads `.trackers/{BASE_NAME}/TASKS.md` for completion status, skips phases where all tasks are `[x]`, and resumes from incomplete phases.

## Next Steps

[If paused]
- Continue with Phase {next-number}: {name}
- Review `.trackers/{BASE_NAME}/TASKS.md` for current progress
