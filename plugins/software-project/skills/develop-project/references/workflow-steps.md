...
**Check 1**: Does `tasks/{BASE_NAME}/TASKS.md` exist?
...
**Check 2** (only if TASKS.md not found): Does `tasks/{BASE_NAME}/plans/{BASE_NAME}-master-plan.md` exist?
...
2. Create directory: `mkdir -p tasks/{BASE_NAME}/plans`
3. Spawn `software:software-architect` agent:

```
subagent_type: "software:software-architect"
...
## Output File Path
Save the master plan to: tasks/{BASE_NAME}/plans/{BASE_NAME}-master-plan.md
...
tasks/{BASE_NAME}/plans/{BASE_NAME}-master-plan.md"
```
...
File: tasks/{BASE_NAME}/plans/{BASE_NAME}-master-plan.md
...
1. Read `tasks/{BASE_NAME}/plans/{BASE_NAME}-master-plan.md`
...
### 4.1 Call software:split-plan Skill Directly

```
Skill: "software:split-plan"
args: "--master-plan-path tasks/{BASE_NAME}/plans/{BASE_NAME}-master-plan.md --base-name {BASE_NAME}"
```

Wait for skill to complete. It will create all 8 phase plan files in `tasks/{BASE_NAME}/plans/`.

Expected files after skill:
```
tasks/{BASE_NAME}/plans/
...
Create `tasks/{BASE_NAME}/TASKS.md` using Write tool:
...
Read the phase plan: `tasks/{BASE_NAME}/plans/{BASE_NAME}-{NN}-{name}.md`
...
```
subagent_type: "software:senior-developer"
...
## Context
- Detailed phase plan: tasks/{BASE_NAME}/plans/{BASE_NAME}-{NN}-{name}.md
- Task list: tasks/{BASE_NAME}/TASKS.md
...
1. Execute comprehensive review:
   ```
   Skill: "software:comprehensive-review"
   ```
...
1. Read `tasks/{BASE_NAME}/TASKS.md` to collect completion counts.

2. Write summary to `tasks/{BASE_NAME}/plans/{BASE_NAME}-SUMMARY.md` — see `references/summary-report.md` for the template.

3. Present to user:
   ```
   Build workflow complete!

   Summary: tasks/{BASE_NAME}/plans/{BASE_NAME}-SUMMARY.md
   Tasks:   tasks/{BASE_NAME}/TASKS.md

   Completed: {count} phases, {count} tasks
   Remaining: {count} phases, {count} tasks (blocked or skipped)

   To resume at any time: run /software:develop-project again with the same requirements file.
   ```
