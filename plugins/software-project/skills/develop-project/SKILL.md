...
2. **Generate Master Plan** — `software:software-architect` agent analyzes requirements and writes master plan
3. **Review Master Plan** — Present summary; wait for user approval (**only user gate**)
4. **Split Master Plan + Build TASKS.md** — Call `software:split-plan` Skill directly (no agents); build `TASKS.md` from phase plans
5. **Execute All Phases** — Run `software:senior-developer` agents per phase (max 3 when ≥3 tasks, 1 when <3 tasks), auto-continue between phases; run post-implementation comprehensive review loop once after all phases complete (max 2 iterations, ask user if issues remain)
...
- **≥ 3 pending tasks** → spawn **3** `software:senior-developer` agents as a team
...
All output goes under `tasks/{BASE_NAME}/`:
...
- **No tracker plugin**: All task tracking via `tasks/{BASE_NAME}/TASKS.md` using Edit tool
